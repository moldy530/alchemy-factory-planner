...
export function calculateProduction(config: PlannerConfig): ProductionNode[] {
    ...
    // Normalize targets
    const finalTargets = targets && targets.length > 0
        ? targets
        : (targetItem && targetRate ? [{ item: targetItem, rate: targetRate }] : []);
...
function solveNode(
    ...
    let outputCount = 0;
    const outputDef = recipe.outputs.find(o => o.name.toLowerCase() === itemName);
    if (outputDef) {
        let count = typeof outputDef.count === 'string' ? parseFloat(outputDef.count) : outputDef.count;
        let percentage = 100;
        if (outputDef.percentage) {
            percentage = typeof outputDef.percentage === 'string' ? parseFloat(outputDef.percentage) : +outputDef.percentage;
        }
    ...
    if (isNursery && ctx.selectedFertilizer) {
        ...
        if (fertilizerItem && item.required_nutrients) {
            ...
            const nutrientsPerSec = (fertilizerItem.nutrients_per_seconds || 0) * fertEffMult;

            let calculatedRate = (60 * nutrientsPerSec / item.required_nutrients) * ctx.speedMultiplier;
            ...
            fertilizerInput = { name: fertilizerItem.name, rate: totalFertilizerRate };
        } else {
    ...
    if (device && device.heat_consuming_speed && device.category !== 'heating') {
        ...
        heatConsumption = (heaterSpeed + deviceHeatSpeed) * 60 * ctx.speedMultiplier * machinesNeeded;
        ...
        if (!isLoop && ctx.selectedFuel) {
            ...
            if (fuelItem && fuelItem.heat_value) {
                const fuelRate = heatConsumption / (fuelItem.heat_value * ctx.fuelMultiplier);
                const fuelNode = solveNode(fuelItem.name.toLowerCase(), fuelRate, ctx, nextVisited);
    ...
    recipe.outputs.forEach(out => {
        if (out.name.toLowerCase() !== itemName) {
            let count = typeof out.count === 'string' ? parseFloat(out.count) : out.count;
            let percentage = 100;
            if (out.percentage) percentage = typeof out.percentage === 'string' ? parseFloat(out.percentage) : +out.percentage;
    ...
    if (!isLoop) {
        recipe.inputs.forEach(input => {
            ...
            // Recurse
            const inputNode = solveNode(input.name.toLowerCase(), inputRate, ctx, nextVisited);
            if (inputNode) {
