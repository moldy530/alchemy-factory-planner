...
import { ReactFlow, Background, Controls, Node, Edge, useNodesState, useEdgesState, Position, MarkerType, Handle, ReactFlowInstance } from '@xyflow/react';
...
function CustomNode({ data }: { data: ProductionNode }) {
    ...
    return (
        <div className={cn(
            ...
            isTarget ? "border-green-500 bg-green-950/20" :
                isMachine ? "border-amber-600/50" : "border-stone-700",
            ...
            <div className="flex justify-between items-start mb-2 border-b border-stone-800 pb-2">
                <span className={cn("font-bold text-sm truncate",
                    isTarget ? "text-green-400" :
                        isMachine ? "text-amber-100" : "text-stone-400"
                )}>
                ...
                <div className="text-right">
                    <div className={cn("text-xs font-mono font-bold",
                        isTarget ? "text-green-400" :
                            isSaturated ? "text-red-400" : "text-amber-400"
                    )}>
                        {nodeData.rate.toLocaleString(undefined, { maximumFractionDigits: 1 })}/m
                    </div>
            ...
            <div className="space-y-1">
                ...
                {isMachine && (
                    <div className="flex items-center gap-2 text-xs text-stone-400">
                        ...
                        <span className="text-stone-200 font-bold">{nodeData.deviceCount.toLocaleString(undefined, { maximumFractionDigits: 2 })}x</span>
            ...
            {/* Handles for Edges */}
            <Handle type="target" position={Position.Left} className="w-3 h-3 bg-stone-500 border-2 border-stone-800" />
            {/* Targets usually don't have source, but we leave it flexible */}
            {!isTarget && (
                <Handle type="source" position={Position.Right} className="w-3 h-3 bg-stone-500 border-2 border-stone-800" />
            )}
        </div>
    );
}
...
export function GraphView({ rootNodes, rootNode, factoryId = 'default' }: GraphViewProps) {
    ...
    // Debug
    console.log(`[GraphView] Render with factoryId=${factoryId}, POS_KEY=${POS_KEY}`);
