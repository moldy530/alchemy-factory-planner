...
if (res4[0]) {
  ...
  const fertInput = res4[0].inputs.find(
    (i) => i.itemName === "Basic Fertilizer",
  );
  if (fertInput) {
    console.log(
      `Fertilizer Input: ${fertInput.itemName} @ ${fertInput.rate}/min`,
    );
  } else {
    console.log("No Fertilizer Input found!");
    console.log(
      "Inputs:",
      res4[0].inputs.map((i) => i.itemName),
    );
  }
}
...
if (res5[0]) {
  ...
  if (fuelInput) {
    console.log(
      `Fuel Input: ${fuelInput.itemName} @ ${fuelInput.rate} (Expected: ~0.277)`,
    );
  } else {
    console.log("No Fuel Input found!");
    console.log(
      "Inputs:",
      res5[0].inputs.map((i) => i.itemName),
    );
  }
}
